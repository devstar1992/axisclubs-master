...
exports.postWithdrawl = async (req, res, next) => {
  ...
  if (amount < 130)
    return res.status(400).json({ error: "Only more than ₹ 130 allowed" });
...
exports.postRecharge = async (req, res, next) => {
  if (req.body.money === "") {
    return res.status(200).json({ error: "Please input correct amount" });
  }
  if (req.body.money < 200) {
    return res.status(200).json({ error: "More than ₹ 200 allowed" });
  }
  const user = await User.findById(req.userFromToken._id);
  const comp = {};
